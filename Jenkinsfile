pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "captainaniii/security-service"  // Your Docker Hub repo/image
        ...
        K8S_DEPLOYMENT = 'security-service'  // Your deployment name from YAML
    ...
    stages {
        stage('Checkout') {
            steps {
                git(
                    branch: 'feature/JWT-FOR-MICROSERVICES',
                    credentialsId: 'github-credentials',  // Your token-based credential ID
                    url: 'https://github.com/iamaniketg/Jwt-authentication.git'  // Changed to HTTPS
                )
            }
        }
                    ...
                    script {
                        ...
                        retry(2) {  // Retry for transient GKE issues
                            sh """
                                ...
                                sed -i 's|image: .*|image: ${fullImage}|g' security-deployment.yaml
                                kubectl apply -f security-configmap.yaml
